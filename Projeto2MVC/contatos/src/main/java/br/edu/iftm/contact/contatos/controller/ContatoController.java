...
public class ContatoController {
    ...
    public String inserirContatos(Contato contato,Model model) {
        Contato contatoDb = dao.getContato(contato.getMatricula());
        if (contatoDb == null) {
    ...
    public String deleteContato(@RequestParam(value = "matricula", required = true) String matricula, Model model) {
        dao.deleteContato(matricula);
        return getContatos(model);
    }

    @RequestMapping("editarContato")
    public String editarContato(@RequestParam(value = "matricula", required = true) String matricula, Model model) {
        Contato contato = dao.getContato(matricula);
